...
struct TProblem									//**D�finition du probl�me:
{
	...
	std::vector<std::vector <int> > Distance;	//**Distance entre chaque paire d'element. NB: Tableau de 0 � N-1.  Indice 0 utilis�.
...
{
	std::vector <bool> Selec;			//**Ensemble de genes: indique si l'element i est s�lectionne (true) ou non (false). NB: Tableau de 0 � N-1.
	long FctObj = -INT_MAX;				//**Valeur de la fonction obj: Sommation des distances entre les objects s�lectionnes
	bool Valide=false;					//**Etat de validit� de la solution (nombre elements selectionnes)
	int NbEvaltoGet;					//**Nombre d'evalutions de solutions necessaires pour trouver cette solution... pour �valuer convergence
};
